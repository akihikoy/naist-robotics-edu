//-------------------------------------------------------------------------------------------
/*! \file    bioloid-test2.cpp
    \brief   bioloid test program 2
...
// usleep(50000);
  ...
  for(int i(1);i<=20;++i)
  {
    if(bioloid.Ping(i)==0)  cerr<<"Dynamixel found: "<<i<<endl;
    else  cerr<<"no Dynamixel: "<<i<<endl;
// usleep(50000);
  }
  for(int i(1);i<=20;++i)
  {
...
// return 0;
  ...
  int skip(0);
  double omega(2.0*M_PI*1), goal, distance_c;
